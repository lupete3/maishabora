...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th>ID</th><th>Membre</th><th>Date Crédit</th><th>Montant</th><th>Solde</th>
                <th>Pénalités</th><th>% Pénalités</th><th>Jours Retard</th>
        ...
        <tbody>
            @foreach ($details as $d)
                <tr>
                    ...
                    <td>{{ $d['days_late'] }}</td>
                    <td>{{ number_format($d['range_1'], 2) }}</td>
                    <td>{{ number_format($d['range_2'], 2) }}</td>
                    <td>{{ number_format($d['range_3'], 2) }}</td>
                    <td>{{ number_format($d['range_4'], 2) }}</td>
                    <td>{{ number_format($d['range_5'], 2) }}</td>
                    <td>{{ number_format($d['range_6'], 2) }}</td>
                    <td>{{ number_format($d['range_7'], 2) }}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
